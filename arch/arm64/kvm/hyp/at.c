...
void __hyp_text __at_switch_to_guest_nvhe(struct kvm_vcpu *vcpu, bool el2_regime)
{
...
void __hyp_text __at_switch_to_host_nvhe(struct kvm_vcpu *vcpu)
{
        struct kvm_cpu_context *host_ctxt;
...
void __hyp_text __at_switch_to_host_vhe(struct kvm_vcpu *vcpu)
{
 ...
 * Switching to host.
 *
 * 1. [nvhe] Restore host vm regs
 *    (We don't save guest vm regs since we didn't make any changes)
 * 2. [both] Restore hcr_el2
 */
static hyp_alternate_select(__at_switch_to_host,
                            __at_switch_to_host_nvhe, __at_switch_to_host_vhe,
                            ARM64_HAS_VIRT_HOST_EXTN);

void __hyp_text __kvm_at_insn(struct kvm_vcpu *vcpu, unsigned long vaddr, bool el2_regime, int sys_encoding)
{
