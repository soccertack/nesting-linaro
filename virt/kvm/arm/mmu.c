...
static void unmap_stage2_pmds(struct kvm *kvm, struct kvm_s2_mmu *mmu,
			      pud_t *pud, phys_addr_t addr, phys_addr_t end)
{
	...
	do {
		...
		if (!pmd_none(*pmd)) {
			if (pmd_thp_or_huge(*pmd)) {
				...
				clear_rmap_pmd(kvm, mmu, (unsigned long)pmd,
					       addr);
...
static void unmap_stage2_puds(struct kvm *kvm, struct kvm_s2_mmu *mmu,
			      pgd_t *pgd, phys_addr_t addr, phys_addr_t end)
{
	phys_addr_t next, start_addr = addr;
	pud_t *pud, *start_pud;
...
static int __kvm_test_age_hva_handler(struct kvm *kvm, struct kvm_s2_mmu *mmu,
				      gpa_t gpa, u64 size, void *data)
{
	pmd_t *pmd;
	pte_t *pte;
...
static int kvm_test_age_hva_handler(struct kvm *kvm, gpa_t gpa, u64 size,
				    void *data)
{
	int young;

	young = __kvm_test_age_hva_handler(kvm, &kvm->arch.mmu, gpa, size,
					   data);

	young |= handle_nested_s2(kvm, gpa, size, __kvm_test_age_hva_handler,
				  data);
